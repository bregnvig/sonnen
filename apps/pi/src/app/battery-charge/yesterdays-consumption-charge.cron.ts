...
export class YesterdaysConsumptionBasedBatteryChargeCronJob {
  ...
  async getOptimalChargeTime(date: DateTime, chargeMinuttes: number, periodBeforeSurplusProductionInHours = 8): Promise<DateTime> {
    ...
    if (prices.length === 0) {
      this.#logger.warn('No price data available. Cannot determine optimal charge time. Starting now.', date.toISO(), chargeMinuttes, periodBeforeSurplusProductionInHours);
      return date;
    }
